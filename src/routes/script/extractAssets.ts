...
/**
 * 从剧本中提取资产（角色/场景/道具），自动存入 o_assets 并建立关联
 * 可在后台静默运行，不阻塞调用方
 */
export async function extractAssetsFromScripts(
  ...
  async function processGroup(group: number[][][]) {
    await Promise.all(group.map(async (itemIds) => {
      ...
      try {
        ...
        const scriptAssetExtraction = promptData?.useData || promptData?.data || "";
  ...
  await processGroup(scriptGroups);
}
...
export default router.post(
  ...
  async (req, res) => {
    ...
    extractAssetsFromScripts(scriptIds, projectId, groupSize).catch((err) => {
      console.error("[extractAssets] 提取失败:", err);
    });
  },
);
