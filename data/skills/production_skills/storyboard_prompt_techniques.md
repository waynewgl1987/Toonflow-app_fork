...
### 核心要求
同一角色在**同一场景/时间段内**的服装描写必须保持统一。服装变化仅发生在**场景切换**或**剧情明确要求换装**时。

### 提示词中的体现

1. **默认服装锚定**：首帧出现某角色时，根据资产定义确定其服装，后续连续分镜中保持该服装描述不变
2. **跨帧一致性**：同一场戏内，角色的服装描述词（如"白色衬衫""校服""礼服"）在提示词中必须一致
3. **场景驱动换装**：仅当场景上下文发生实质性变化（环境切换、时间跳跃）或剧情写明换装时，才修改服装描述
4. **显式标注换装点**：换装时在提示词中显式描述新服装，不得从旧服装"渐变"到新服装
5. **换装后锁定**：换装后，后续连续分镜锁定新服装，直至下一次场景切换或剧情换装
...
❌ 错误：同一场景服装突变
  分镜1: "角色A穿着白色衬衫坐在办公桌前..."
  分镜2: "角色A穿着黑色T恤起身走向窗边..."  ← 无场景切换，不应换装

✅ 正确：场景切换换装
  (办公室场景结束) → (派对场景开始)
  分镜5: "角色A换上了黑色礼服，端着酒杯站在人群中..."
```
...
### 核心要求
所有角色在提示词中必须**精确保持其固有性别特征**，男女性别特征不可混淆、不可模糊化处理。

### 性别锚定规则

1. **首帧性别锚定**：角色首次出现时，必须使用明确的性别称谓和性别特征描述：
   - 女性角色：使用"女性/女人/女孩/女士/姑娘"等明确女性称谓，描述女性面容特征、体态特征
   - 男性角色：使用"男性/男人/男孩/男士/小伙"等明确男性称谓，描述男性面容特征、体态特征
   
2. **跨帧性别一致性**：同一角色在所有分镜中，性别称谓、性别特征描述、轮廓特征必须完全一致：
   - 女性 → 始终使用"她/女性/女人/女孩"，提示词中始终包含女性面容特征词
   - 男性 → 始终使用"他/男性/男人/男孩"，提示词中始终包含男性面容特征词

3. **性别特征显式化**：提示词中必须包含至少2项该角色性别的身体/面容特征词：
   - 女性特征：轻柔面部轮廓、柔和下颌线、细腻皮肤、明显睫毛、唇形清晰、纤细颈部、肩部曲线
   - 男性特征：棱角分明面部轮廓、清晰下颌线/下颌角、粗犷皮肤纹理、喉结可见、宽阔肩部、骨节分明的手

4. **禁止中性化**：不允许使用"人物/这个人/该角色/那人"等性别模糊称谓。不允许使用"年轻白领""穿西装的职场人"等省略性别的描述。每个角色在每帧提示词中都必须有明确性别标志。

5. **参照资产描述**：资产库中角色的性别定义（来自 `art_character.md` 的类型分类）是唯一标准，提示词不得偏离或模糊资产定义的性别特征。

### 验证规则
- 每个角色名称出现时，检查其是否带有明确的性别称谓
- 检查同一角色在不同分镜的提示词中，性别特征描述是否一致
- 场景中有多人时，确保每个人的性别称谓可明确区分
