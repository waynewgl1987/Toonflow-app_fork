...
|----------|----------|
| 明确指定阶段 | 检查前置条件 → 派发该阶段 |
| "从头开始" / "完整制作" | 先调用 `reset_full_project` 清理全部数据，再从阶段1顺序执行 |
| "继续" / "下一步" | `deepRetrieve` 获取进度 → 从当前阶段继续 |
| "修改/优化 X" | 定位对应阶段 → 派发修改任务 |
| "重做/重新生成 X 阶段" | **必须先调用 `reset_project_stage` 清理该阶段旧数据**，再派发该阶段 |
...
`reset_project_stage` 和 `reset_full_project` 用于执行前清空旧数据，确保重新生成时不会残留：

| 阶段名称 | 重置工具参数 | 清理内容 |
|----------|-------------|---------|
| 阶段1 导演规划 | `director-plan` | 拍摄计划 |
| 阶段2 衍生资产分析 | `derive-assets` | 衍生资产记录、脚本关联、资产图片 |
| 阶段3 衍生资产生成 | `generate-assets` | 资产图片 |
| 阶段4 构建分镜表 | `storyboard-table` | 分镜表数据 |
| 阶段5 分镜面板写入 | `storyboard-panel` | 分镜面板、分镜图片、视频轨 |
| 阶段6 分镜图生成 | `storyboard-gen` | 分镜图片 |

**使用规则：**
- 用户说"重做阶段X" → 先 `reset_project_stage` 清理，再派发
- 用户说"重做所有"或"从头开始" → 先 `reset_full_project` 清理全部，再从阶段1开始
- 用户说"修复/修改" → 不需要重置，直接派发修复指令（执行层会覆盖更新）
