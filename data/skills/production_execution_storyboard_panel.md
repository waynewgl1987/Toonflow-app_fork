...
**核心原则**：同一角色在同一场景/同一时间段内，服装必须保持一致。服装变化只发生在场景切换或剧情明确要求换装时。

**约束细则**：

1. **默认态锁定**：角色的父资产已定义其基础服装（如"校服""西装""古装"），该服装即为该角色的**默认态**。在分镜面板中，除非剧情明确指示换装或场景环境切换，否则角色应始终以默认服装出现
2. **场景决定服装**：换装的判断依据是**场景上下文的变化**：
   - 同一场戏、同一场景内 → 服装**不变**
   - 场景切换（如从办公室→卧室、从日景→夜景派对）→ 可以随场景**合理更换**
   - 剧情明确写"换衣服""穿礼服""脱下某件"等 → 按剧情**执行换装**
3. **换装规则**：当需要换装时，在 `videoDesc` 中显式描述服装变化（如"角色A已换上睡衣"），并在 `prompt` 中通过服装描述词体现。同一场内的多组分镜，服装描述必须保持一致
4. **提示词一致性**：同一角色的服装描述词在连续分镜中必须保持一致（除非场景/剧情要求换装），禁止同一场景内出现同一角色一会穿裙装一会穿裤装等不一致情况
5. **例外**：梦境、幻想、回忆闪回等特殊叙事段落，可以按剧情需要自由设定服装，不受此约束

### 🚻 角色性别锚定约束（重要）

**核心原则**：每个角色在分镜面板的每一帧中，性别特征必须精确锚定，不可混淆、不可模糊。

**约束细则**：

1. **首次强制锚定**：角色在第一个分镜中必须明确定性：女性角色标注"女性/女人/女孩"，男性角色标注"男性/男人/男孩"
2. **全程性别一致**：同一角色在所有分镜中，性别称谓、面部特征、身体轮廓必须保持一致
3. **特征词标配**：每个角色的提示词必须包含至少1项性别特征词：
   - 女性：`女性面容柔和`、`纤细脖颈`、`清晰唇形`、`柔和下颌线`等
   - 男性：`男性轮廓分明`、`清晰下颌角`、`喉结可见`、`宽阔肩部`等
4. **禁止省略性别**：任何角色的任何帧中，不得使用"该人物""那人""穿XX的人"等省略性别的描述
5. **校验**：所有分镜写入完成后，逐条检查每个角色的性别标注是否存在且一致
