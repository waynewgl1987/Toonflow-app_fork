...
- **服装一致性原则（重要）**：角色的默认资产已定义其基础服装。分镜写作阶段，若无场景切换或剧情需要的换装，角色应保持同一套服装贯穿始终。只有在以下情况才允许创建服装类衍生：
  - **场景环境转换**（如室内→室外、白天→夜晚、工作场所→休闲场所等自然环境/场景类型变化）
  - **剧情明确要求换装**（剧本写明"换上礼服""脱下外套"等）
  - **身份/形态切换**（如变身战斗形态、制服变便装）
  - 除此之外的普通连贯场景中，角色服装应**保持统一**，不做额外的服装衍生
